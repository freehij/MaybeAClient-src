...
public class GuiChat extends GuiScreen {
    ...
    protected void keyTyped(char var1, int var2) {
    	if(var2 == Keyboard.KEY_UP) {
    		if(lastHistoryIndex > 0 && lastHistoryIndex <= history.size()) this.message = history.get(--lastHistoryIndex);
    	} else if(var2 == Keyboard.KEY_DOWN) {
    		if((lastHistoryIndex+1) >= history.size()) {
    			this.message = "";
    			if(++lastHistoryIndex > history.size()) --lastHistoryIndex;
    		}else if(lastHistoryIndex >= 0 && lastHistoryIndex < history.size()) this.message = history.get(++lastHistoryIndex);
    	} else if (var2 == 1) {
